...
func (l *DeleteUpgradeLnxVersionLogic) DeleteUpgradeLnxVersion(req *types.IDsReq) (*types.BaseMsgResp, error) {
	...
	for _, id := range req.Ids {
		data, err := l.svcCtx.DB.UpgradeLnxVersion.Get(l.ctx, id)
		if err != nil {
		...
		// 删除状态
		predicates = append(predicates, upgradelnxupgradestrategy.LnxVersionID(id))
	...
	err = l.svcCtx.DB.UpgradeLnxVersion.Update().
		Where(upgradelnxversion.IDIn(req.Ids...), upgradelnxversion.CompanyIDIn(companyID)).
