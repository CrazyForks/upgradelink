...
func (l *DeleteUpgradeMacUpgradeStrategyLogic) DeleteUpgradeMacUpgradeStrategy(req *types.IDsReq) (*types.BaseMsgResp, error) {
	...
	for _, id := range req.Ids {
		data, err := l.svcCtx.DB.UpgradeMacUpgradeStrategy.Get(l.ctx, id)
		if err != nil {
	...
	if err := base.WithTx(l.ctx, l.svcCtx.DB, func(tx *ent.Tx) error {
		...
		for i := 0; i < len(req.Ids); i++ {
			...
			// 获取策略信息
			strategyData, err := l.svcCtx.DB.UpgradeMacUpgradeStrategy.Get(l.ctx, req.Ids[i])
			if err != nil {
				return err
			}
			...
			// 删除策略信息
			err = l.svcCtx.DB.UpgradeMacUpgradeStrategy.Update().
				Where(upgrademacupgradestrategy.IDIn(req.Ids...), upgrademacupgradestrategy.CompanyIDIn(companyID)).
				SetNotNilIsDel(&intDel).
				Exec(l.ctx)
			if err != nil {
				return err
