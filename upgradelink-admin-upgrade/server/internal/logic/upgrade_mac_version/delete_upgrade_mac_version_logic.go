...
func (l *DeleteUpgradeMacVersionLogic) DeleteUpgradeMacVersion(req *types.IDsReq) (*types.BaseMsgResp, error) {
	...
	for _, id := range req.Ids {
		data, err := l.svcCtx.DB.UpgradeMacVersion.Get(l.ctx, id)
		if err != nil {
		...
		// 删除状态
		predicates = append(predicates, upgrademacupgradestrategy.MACVersionID(id))
	...
	err = l.svcCtx.DB.UpgradeMacVersion.Update().
		Where(upgrademacversion.IDIn(req.Ids...), upgrademacversion.CompanyIDIn(companyID)).
