...
func (l *DeleteUpgradeWinVersionLogic) DeleteUpgradeWinVersion(req *types.IDsReq) (*types.BaseMsgResp, error) {
	...
	for _, id := range req.Ids {
		data, err := l.svcCtx.DB.UpgradeWinVersion.Get(l.ctx, id)
		if err != nil {
		...
		// 删除状态
		predicates = append(predicates, upgradewinupgradestrategy.WinVersionID(id))
	...
	err = l.svcCtx.DB.UpgradeWinVersion.Update().
		Where(upgradewinversion.IDIn(req.Ids...), upgradewinversion.CompanyIDIn(companyID)).
