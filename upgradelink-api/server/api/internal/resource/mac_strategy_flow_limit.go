...
const (
	CacheKeyGetMacFlowLimitStrategyListByIds              = PREFIX + "MAC_FLOW_LIMIT_LIST:IDS:%v"
	CacheKeyGetMacFlowLimitStrategyIncrementByIdAndPeriod = PREFIX + "MAC_FLOW_LIMIT_STRATEGY_INCREMENT:ID:%v:PERIOD:%v"
)

// GetMacFlowLimitStrategyListByIds
// 通过 ids 获取到流控策略list
func (c *Ctx) GetMacFlowLimitStrategyListByIds(ctx context.Context, ids string) ([]*model.UpgradeMacUpgradeStrategyFlowLimitStrategy, error) {

	cacheKey := fmt.Sprintf(CacheKeyGetMacFlowLimitStrategyListByIds, ids)
	// 内存缓存
	v, err := c.localCache.Take(cacheKey, func() (interface{}, error) {
		...
		// 构建 SQL 查询语句
		query := fmt.Sprintf("select * from upgrade_mac_upgrade_strategy_flow_limit_strategy "+
			"where id in ( %s ) "+
			"AND enable = 1 "+
		...
		// sql 缓存查询
		var strategyList []*model.UpgradeMacUpgradeStrategyFlowLimitStrategy
	...
	if v != nil {
		strategyList := v.([]*model.UpgradeMacUpgradeStrategyFlowLimitStrategy)
...
// MacFlowLimitStrategyIncrement 判断当前请求在流控策略要求的数量范围内  使用 redis 计数器方式实现
func (c *Ctx) MacFlowLimitStrategyIncrement(ctx context.Context, flowLimitStrategy *model.UpgradeMacUpgradeStrategyFlowLimitStrategy) (bool, error) {
	...
	// 构建 Redis 键名
	cacheKey := fmt.Sprintf(CacheKeyGetMacFlowLimitStrategyIncrementByIdAndPeriod, flowLimitStrategy.Id, period)
