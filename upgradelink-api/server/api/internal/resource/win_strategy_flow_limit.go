...
const (
	CacheKeyGetWinFlowLimitStrategyListByIds              = PREFIX + "WIN_FLOW_LIMIT_LIST:IDS:%v"
	CacheKeyGetWinFlowLimitStrategyIncrementByIdAndPeriod = PREFIX + "WIN_FLOW_LIMIT_STRATEGY_INCREMENT:ID:%v:PERIOD:%v"
)

// GetWinFlowLimitStrategyListByIds
// 通过 ids 获取到流控策略list
func (c *Ctx) GetWinFlowLimitStrategyListByIds(ctx context.Context, ids string) ([]*model.UpgradeWinUpgradeStrategyFlowLimitStrategy, error) {

	cacheKey := fmt.Sprintf(CacheKeyGetWinFlowLimitStrategyListByIds, ids)
	// 内存缓存
	v, err := c.localCache.Take(cacheKey, func() (interface{}, error) {
		...
		// 构建 SQL 查询语句
		query := fmt.Sprintf("select * from upgrade_win_upgrade_strategy_flow_limit_strategy "+
			"where id in ( %s ) "+
			"AND enable = 1 "+
		...
		// sql 缓存查询
		var strategyList []*model.UpgradeWinUpgradeStrategyFlowLimitStrategy
	...
	if v != nil {
		strategyList := v.([]*model.UpgradeWinUpgradeStrategyFlowLimitStrategy)
...
// WinFlowLimitStrategyIncrement 判断当前请求在流控策略要求的数量范围内  使用 redis 计数器方式实现
func (c *Ctx) WinFlowLimitStrategyIncrement(ctx context.Context, flowLimitStrategy *model.UpgradeWinUpgradeStrategyFlowLimitStrategy) (bool, error) {
	...
	// 构建 Redis 键名
	cacheKey := fmt.Sprintf(CacheKeyGetWinFlowLimitStrategyIncrementByIdAndPeriod, flowLimitStrategy.Id, period)
